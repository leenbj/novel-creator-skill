...
python3 scripts/plot_rag_retriever.py query --project-root <小说项目目录> --query "<新剧情>" --top-k 4 --auto-build
```

脚本会在 `00_memory/retrieval/` 下生成索引与写前上下文建议文件；`build` 默认是增量构建，只重算变更章节。  
`query` 默认启用条件触发（轻场景跳过）、片段级召回和查询缓存；可用 `--force` 强制检索。
...
- 机制：默认条件触发 + 查询缓存；可用 `--force`、`--no-cache`、`--no-conditional` 覆盖默认策略。
