...
## 更新说明（v6.2）
- 将“每章质量闭环”升级为“脚本硬校验闭环”，新增 `/门禁检查`。
- 强制顺序固定为：`/更新记忆` → `/检查一致性` → `/风格校准` → `/校稿` → `/门禁检查`。
- 新增门禁产物规范与校验脚本，未通过门禁时章节禁止进入下一章。
- 保留原有命令全部能力，并把详细说明下沉到 `references/`，减少主说明上下文开销。
...
## 4. 三种工作模式
1. 从模糊想法开书：`/写全篇` → `/建库` → `/写作` → 章节闭环。
2. 从样章仿写开书：`/仿写` → `/风格提取` → `/题材选风格` → `/写作` → 章节闭环。
3. 已有项目续写：`/续写` → `/写作` 或 `/批量写作` → 章节闭环。
...
- 何时使用：每章门禁最后一步，强制执行。
- 输入：项目目录、章节文件。
- 输出：`gate_result.json`（`passed=true` 才可进入下一章）。
